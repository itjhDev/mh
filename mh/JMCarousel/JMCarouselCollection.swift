...
//            cell.bkgImageView.sd_setImageWithURL(NSURL(string: (imgURLArray?[i])!), placeholderImage: UIImage(named: "holder"))
            ...
            cell.bkgImageView.yy_setImageWithURL(NSURL(string: (imgURLArray?[i])!), options: [.SetImageWithFadeAnimation,.ProgressiveBlur])
        }
    ...
    */
    ...
    {
        timer = NSTimer.scheduledTimerWithTimeInterval(pageStepTime, target: self, selector: #selector(JMCarouselCollection.nextImage), userInfo: nil, repeats: true)
